<li
        ...
        name="{{ $filter->name }}"
        entity="{{$filter->options['entity']}}"
        id="export-button"
>Exel Export
</li>

{{--<a href="order-export" id="download-link">EXPORT</a>--}}
    ...
    <script>
        jQuery(document).ready(function ($) {
            ...
            function makeOP(elem) {
                ...
                var download_link = $("#export-button");
                var splitUrl = new_url.split("?");

                //console.log(download_link);
                // download_link.attr("href", 'order-export' + '?' + splitUrl[1]);
                // console.log('order-export' + '?' + splitUrl[1]);

                window.location = download_link.attr('entity') + '?' + splitUrl[1];
